...
export const createComment = async (req : Request, res : Response) : Promise<Response> => {
	...
	try {
		...
		return res.status(201).location("/posts/"+result._id+"/comments").set("Content-Type", "text/plain").send("Created!");
	}
	catch (e) {
...
export const deleteComment = async (req : Request, res : Response) : Promise<Response> => {
	...
	await Post.findOneAndUpdate({ _id : id, isDeleted : false, comments : { $elemMatch : {_id : idc} }},
